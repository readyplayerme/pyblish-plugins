...
from readyplayerme.pyblish_plugins.action_file_save import FileSave
from readyplayerme.pyblish_plugins.action_file_showinexplorer import FileShowInExplorer
...
class _FileHierarchy(pyblish.api.ContextPlugin):
    ...
    def process(self, context):
        ...
        if current_file.parent.stem != "working-files":
            self.log.error(
                f"{report_msg}File must be saved in the subfolder 'working-files' of an asset's folder, "
            ...
            self.log.error(
                f"{report_msg}File name '{current_file.stem}' does not match asset folder '{asset_folder}'."
        ...
        if asset_type not in const.KNOWN_ASSET_TYPES:
            ...
            self.log.error(f"{report_msg}First part of the '-' separated file-name must be one of: {known_types_str}.")
...
def get_human_readable_patterns(key: Union[str, List[str]]) -> str:
    ...
    patterns = [p.replace("^", "").replace("$", "").replace("(", "").replace(")", "") for p in get_patterns(key)]
